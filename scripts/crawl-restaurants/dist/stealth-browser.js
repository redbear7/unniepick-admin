...
/** 프록시를 포함한 실행 옵션 반환 */
export function makeLaunchArgs(proxy) {
    return {
        headless: true,
        args: BASE_ARGS,
        ...(proxy ? { proxy } : {}),
 ...
 */
export async function injectStealth(target) {
 ...
 */
export async function waitForApollo(page) {
    try {
...
export async function waitForContent(page) {
    try {
        await page.waitForFunction(() => document.body.innerText.length > 500, { timeout: 5_000 });
    }
    catch { }
}

 ...
 */
export async function verifyWithNaver(page, restaurants) {
    const verified = [];
    const failed = [];
    ...
    for (let i = 0; i < restaurants.length; i++) {
        ...
        try {
            ...
            const naverData = await page.evaluate((targetName) => {
            ...
            console.log(`    [${i + 1}/${restaurants.length}] ✗ ${r.name} — 에러: ${e.message}`);

 ...
 */
export async function crawlPublicData() {
    const results = [];
    ...
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            // 창원시 필터링 (창원, 마산, 진해 포함)
            if (!addr.includes('창원') && !addr.includes('마산') && !addr.includes('진해'))
                continue;
            const status = cols[statusIdx] ?? '';
            if (status && status.includes('폐업'))
                continue;
        ...
        console.log(`    [공공데이터] CSV 에러: ${e.message}`);
        return await crawlLocalDataApi();
    }
...
/** LOCALDATA API 폴백 */
async function crawlLocalDataApi() {

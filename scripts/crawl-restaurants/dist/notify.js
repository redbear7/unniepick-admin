...
export async function notifyNewRestaurants(restaurants, keyword) {
...
export async function notifyClosureReport(report) {
    ...
    if (report.newlyClosed.length) {
        ...
        if (report.newlyClosed.length > 10)
            text += `\\.\\.\\. 외 ${report.newlyClosed.length - 10}개\n`;
        text += '\n';
    }
    if (report.newlySuspected.length) {
        ...
        if (report.newlySuspected.length > 5)
            text += `\\.\\.\\. 외 ${report.newlySuspected.length - 5}개\n`;
        text += '\n';
    }
...
export async function notifyDailySummary(total, newCount, keywords) {
    const now = new Date().toLocaleString('ko-KR');
...
async function sendTelegram(text) {
    try {
        ...
        console.log('[telegram] 에러:', e.message);
    }
}

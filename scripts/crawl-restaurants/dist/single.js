...
import { upsertRestaurants } from './storage.js';
...
function writeResult(payload) {
    try {
        ...
        console.error('[single] 결과 파일 쓰기 실패:', e.message);
    }
}
// ── Apollo 캐시에서 첫 번째 결과만 추출 ──────────────────────
async function getFirstResult(page, query) {
    console.log(`🔍 네이버 플레이스 검색: "${query}"`);
    await page.goto(`https://pcmap.place.naver.com/restaurant/list?query=${encodeURIComponent(query)}`, { waitUntil: 'networkidle', timeout: 25_000 });
...
try {
    ...
    try {
        ...
        if (detail.business_hours) {
            store.business_hours = detail.business_hours;
            console.log(`  영업시간: ${detail.business_hours}`);
        }
        if (detail.business_hours_detail)
            store.business_hours_detail = detail.business_hours_detail;
        if (detail.website_url) {
            store.website_url = detail.website_url;
            console.log(`  홈페이지: ${detail.website_url}`);
        }
        if (detail.instagram_url) {
            store.instagram_url = detail.instagram_url;
            console.log(`  인스타: ${detail.instagram_url}`);
        }
        if (detail.menu_items?.length) {
            store.menu_items = detail.menu_items;
            console.log(`  메뉴: ${detail.menu_items.length}개`);
        }
    }
    catch (e) {
        console.log(`  상세 정보 에러: ${e.message}`);
    }
    // 3. 리뷰 분석 (옵션)
    if (analyzeReviews) {
        console.log('\n리뷰 분석 중...');

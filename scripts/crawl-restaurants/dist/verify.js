...
import { getRestaurantsForVerification, updateClosureStatus, } from './storage.js';
import { notifyClosureReport } from './notify.js';
/**
 * 네이버 플레이스 페이지 재접근으로 영업 상태 확인
 */
async function checkNaverPlace(page, placeId) {
    try {
        const res = await page.goto(`https://pcmap.place.naver.com/restaurant/${placeId}/home`, { waitUntil: 'networkidle', timeout: 15_000 });
        ...
        for (const p of TRUE_CLOSED_PATTERNS) {
            ...
            if (m)
                return { status: 'closed_text', reason: m[0] };
        }
        ...
        return { status: 'network_error', reason: e.message };
    }
}
/** 메인 검증 루틴 */

...
async function debug() {
    ...
    const visibleNames = await page.$$eval('[class*="TYaxT"]', (els) => els.map((el) => el.textContent?.trim() ?? ''));
    ...
    const apolloEntries = await page.evaluate(() => {
        ...
        for (const [key, val] of Object.entries(apollo)) {
            ...
            if (m)
                entries.push({ key, id: m[1], name: val.name ?? '' });
        }
        return entries;
    });
    ...
    const apollo = await page.evaluate(() => window.__APOLLO_STATE__);

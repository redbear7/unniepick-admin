...
async function debug() {
    ...
    // 모든 li 확인
    const cwLis = await p1.$$eval('li', (els) => els.slice(0, 30).map((el) => ({
    ...
    // dl/dd 패턴 확인
    const cwDls = await p1.$$eval('dl, dd, dt, .item, .card, article, .cont', (els) => els.slice(0, 10).map((el) => ({
        tag: el.tagName,
        class: el.className.slice(0, 80),
    ...
    try {
        ...
        console.log('body 텍스트:\n', brText);
        // 모든 a 태그 중 맛집 이름 후보
        const brItems = await p2.$$eval('a, div, li, article', (els) => els.slice(0, 50).map((el) => ({
            tag: el.tagName,
            class: el.className.slice(0, 80),

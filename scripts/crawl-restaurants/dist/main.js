...
import { upsertRestaurants, getStats, getExistingIds, getActiveKeywords, updateKeywordStatus, } from './storage.js';
...
async function crawl(keywords) {
    ...
    const crawler = new PlaywrightCrawler({
        ...
        launchContext: {
            ...
            launchOptions: makeLaunchArgs(proxy),
        },
        ...
        async requestHandler({ page, request, log }) {
            const { kw } = request.userData;
            log.info(`🔍 "${kw.keyword}" (리뷰 상세 분석)`);
            if (!kw.id.startsWith('adhoc-'))
                ...
                for (let i = 0; i < restaurants.length; i++) {
                    ...
                    try {
                        ...
                        if (detail.business_hours)
                            r.business_hours = detail.business_hours;
                        if (detail.business_hours_detail)
                            r.business_hours_detail = detail.business_hours_detail;
                        if (detail.website_url)
                            r.website_url = detail.website_url;
                        if (detail.instagram_url)
                            r.instagram_url = detail.instagram_url;
                        if (detail.menu_items?.length)
                            r.menu_items = detail.menu_items;
            ...
            for (const r of restaurants) {
                if (r.image_url) {
                    r.image_url_original = r.image_url;
...
async function collectFromApollo(page, query, limit = 0) {
    const allResults = [];
    const seenIds = new Set();
    const maxPage = limit > 0 ? Math.max(1, Math.ceil(limit / 15)) : 5;
    // 첫 요청 전 인간형 딜레이 (3~8초) — 봇 감지 회피
    await humanDelay(3000, 8000);
    await page.goto(`https://pcmap.place.naver.com/restaurant/list?query=${encodeURIComponent(query)}`, { waitUntil: 'networkidle', timeout: 40_000 });
    // 429/차단 페이지 감지
    const statusCode = await page.evaluate(() => {
        const body = document.body?.innerText ?? '';
        if (body.includes('429') || body.includes('Too Many Requests') || body.includes('차단'))
            return 429;
        return 200;
    });
    if (statusCode === 429) {
    ...
    for (let pageNum = 1; pageNum <= maxPage; pageNum++) {
        ...
        for (const item of items) {
            if (seenIds.has(item.naver_place_id))
                continue;
            seenIds.add(item.naver_place_id);
            item.tags = inferTags(item.category ?? '', item.name);
            item.unniepick_category = normalizeToUnniepick(item.category);
        ...
        if (pageNum < maxPage) {
            ...
            const prevFirstName = await page.$eval('[class*="TYaxT"]', (el) => el.textContent?.trim() ?? '').catch(() => '');
            await nextBtn.click();
...
export async function crawlDetailInfo(page, placeId) {
    // ── 1. 홈 탭 ──
    ...
    const homeData = await page.evaluate(() => {
        ...
        if (!businessHours) {
            ...
            for (const l of lines) {
                if (NOISE.some(n => l.includes(n)))
                    continue; // UI 텍스트 제외
                if (!timePattern.test(l))
                    continue; // 실제 시간 범위(~) 없으면 제외
                if (!dayPattern.test(l) && !/매일|평일|주말/.test(l) && !/\d/.test(l.slice(0, 3)))
                    continue; // 요일/시간 맥락 없으면 제외
                if (l.startsWith('http') || l.includes('전화') || l.includes('주소'))
                    continue;
                if (l.length > 100)
                    continue; // 너무 긴 줄 제외 (UI 노이즈)
                if (!hourLines.includes(l))
                    hourLines.push(l); // 중복 제거
                if (hourLines.length >= 5)
                    break;
            }
        ...
        // Apollo에서 URL 추출
        for (const [, val] of Object.entries(apollo)) {
            const url = val?.url ?? val?.homepageUrl ?? '';
            if (!url.startsWith('http'))
                continue;
            if (url.includes('instagram.com') && !instagramUrl)
                instagramUrl = url;
            else if (!url.includes('naver') && !url.includes('kakao') && !websiteUrl)
                websiteUrl = url;
        }
        ...
        if (!instagramUrl || !websiteUrl) {
            ...
            for (const u of found) {
                if (u.includes('instagram.com') && !instagramUrl)
                    instagramUrl = u.replace(/[,.]+$/, '');
...
// ── 리뷰 상세 분석 (새로오픈 전용) ─────────────────────────
async function crawlReviews(page, placeId) {
    // ── 방문자 리뷰 페이지 ──
    ...
    const visitorData = await page.evaluate(() => {
        ...
        for (const line of lines) {
            ...
            if (!name || name.length > 10 || count < 1)
                continue;
    ...
    const blogReviews = await page.evaluate(() => {
        ...
        document.querySelectorAll('a[href]').forEach(a => {
            const href = a.href ?? '';
            if (!href.includes('blog.naver.com') && !href.includes('m.blog.naver.com') && !href.includes('post.naver.com'))
                return;
            const text = a.textContent?.trim() ?? '';
            anchorLinks.push({ href, text });
        });
        function findLink(title) {
            if (!title || !anchorLinks.length)
                return '';
            const exact = anchorLinks.find(a => a.text === title);
            if (exact)
                return exact.href;
            const partial = anchorLinks.find(a => a.text.length >= 10 && (title.includes(a.text) || a.text.includes(title.slice(0, 15))));
            return partial?.href ?? '';
        }
        ...
        for (const line of lines) {
            ...
            if (dateLong.test(line))
                continue;
            if (line.length < 5 || skipPatterns.test(line))
                continue;
            if (line === '다음' || line === '이전' || line === '안내')
                continue;
            buffer.push(line);
        }
...
async function runWithKeywords(keywords, adhoc = false) {
    if (!keywords.length) {

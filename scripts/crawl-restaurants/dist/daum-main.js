...
import { getActiveKeywords, updateKeywordStatus, getExistingKakaoIds, upsertKakaoRestaurants, } from './storage.js';
import { normalizeToUnniepick } from './category-map.js';
import { notifyNewRestaurants } from './notify.js';
// ── Daum 로컬 검색 URL ────────────────────────────────────────────────────────
function daumSearchUrl(query, page = 1) {
...
// ── 단일 페이지에서 업체 추출 ────────────────────────────────────────────────
async function extractPlaces(page, keyword) {
    return page.evaluate((kw) => {
        ...
        for (const el of containers) {
            ...
            // 업체명
            const nameEl = el.querySelector('.tit_place, .link_name, .tit_g a, [class*="place_name"], [class*="name"]');
            const name = nameEl?.textContent?.trim() ?? '';
            if (!name)
                continue;
            // 카테고리
            const catEl = el.querySelector('.txt_type, .txt_category, [class*="category"], .category');
            const category = catEl?.textContent?.trim() ?? '';
            // 주소
            const addrEl = el.querySelector('.txt_location, .txt_addr, [class*="address"], .address');
            const address = addrEl?.textContent?.trim() ?? '';
            // 전화번호
            const phoneEl = el.querySelector('.num_call, .tel, [class*="phone"], .phone');
...
// ── 키워드 하나 크롤링 ───────────────────────────────────────────────────────
async function collectByKeyword(browser, keyword, maxPages = 3) {
...
async function crawl(keywords) {
    ...
    if (existingIds.size > 0 && newPlaces.length > 0) {
        for (const kw of keywords) {
            ...
            if (kwNew.length > 0)
                await notifyNewRestaurants(kwNew, `[다음] ${kw.keyword}`);
        }
    }

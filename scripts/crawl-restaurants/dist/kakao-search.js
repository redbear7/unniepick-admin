...
// ── 단일 페이지 검색 ──────────────────────────────────────────────────────────
export async function searchKakaoKeyword(query, page = 1, opts = {}) {
    const apiKey = process.env.KAKAO_REST_API_KEY;
    if (!apiKey)
        throw new Error('KAKAO_REST_API_KEY 환경변수가 없어요.');
    ...
    if (opts.radius != null && opts.x != null)
...
export async function searchKakaoAll(query, opts = {}) {
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        // limit 달성 시 조기 종료
        if (all.length >= limit || result.isEnd)
            break;
...
export function kakaoGroupToCategory(groupCode, categoryName) {
    // 상세 카테고리에서 마지막 뎁스 추출 (예: "음식점 > 한식 > 설렁탕" → "설렁탕")
    const parts = categoryName.split('>').map(s => s.trim());
    if (parts.length >= 3)
        return parts[parts.length - 1]; // 가장 상세한 카테고리
    if (parts.length === 2)
        return parts[1];
    // 그룹 코드 fallback
    const codeMap = {

...
async function naverLogin(page) {
    ...
    try {
        ...
        console.log('⚠️  로그인 오류:', e.message);
        return false;
    }
}
...
async function extractPlaceIdsFromFolder(folderUrl) {
    // 폴더 해시 추출 (URL 마지막 path segment)
    const hashMatch = folderUrl.match(/folder\/([a-f0-9]+)/);
    if (!hashMatch)
        throw new Error(`폴더 URL에서 hash를 추출할 수 없습니다: ${folderUrl}`);
    ...
    while (true) {
        ...
        start += LIMIT;
        if (start >= total || items.length < LIMIT)
            break;
...
async function fetchPlaceBasicInfo(page, placeId) {
    try {
...
// 3. 신규 업체 상세 크롤링
const browser2 = await stealthChromium.launch(LAUNCH_ARGS);
...
for (let i = 0; i < newItems.length; i++) {
    ...
    try {
        // 기본 정보 수집
        let store = await fetchPlaceBasicInfo(page2, item.placeId);
        if (!store) {
            store = {

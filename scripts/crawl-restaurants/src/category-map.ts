...
export type UnniepickCategory = typeof UNNIEPICK_CATEGORIES[number];

// ── 매핑 규칙 (순서가 우선순위) ───────────────────────────────────────────────
// 앞쪽 규칙이 이길 때까지 순서대로 검사.
// 중복되는 키워드(예: 초밥)는 의도한 카테고리 규칙을 먼저 배치.

const RULES: Array<{ result: UnniepickCategory; keywords: string[] }> = [
  {
    result: '카페',
 ...
 */
export function normalizeToUnniepick(raw: string | null | undefined): UnniepickCategory {
  if (!raw) return '기타';
...
export function kakaoMidCategory(groupCode: string, categoryName: string): string {

 ...
 */
export function generateHexGrid(region: Region, defaultRadius: number): GridPoint[] {
...
// ── 단일 격자점 × 카테고리 전체 수집 ─────────────────────────────
async function collectFromPoint(point: GridPoint, categoryCode: string): Promise<KakaoPlace[]> {
  ...
  for (let page = 1; page <= 3; page++) {
    ...
    try {
      ...
      console.error(`  ⚠️  ${point.name} p${page}: ${(e as Error).message}`);
      break;
    }
  }
  return results;
...
async function main() {
  ...
  if (DRY_RUN) { console.log('✅ DRY-RUN 완료'); return; }

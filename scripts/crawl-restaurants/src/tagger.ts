...
import type { RestaurantData, ReviewKeyword, MenuKeyword, BlogReview } from './storage.js';
...
export interface AutoTags {
  ...
  service:         string[];
  facilities:      string[];
  priceRange:      string[];   // 0 or 1 element
  mealTime:        string[];
  location:        string[];
  characteristics: string[];
  [key: string]:   string[];   // Record<string, string[]> 호환 (빅데이터 분석용 확장 허용)
}

export const AUTO_TAG_DIMENSIONS: Record<keyof AutoTags, string> = {
  foodType:        '음식 유형',
  atmosphere:      '분위기',
  service:         '서비스',
  facilities:      '편의시설',
...
type KwMap = Array<{ tag: string; keywords: string[] }>;

const FOOD_TYPE_MAP: KwMap = [
...
const CHARACTERISTIC_MAP: KwMap = [
...
function matchKeywords(text: string, map: KwMap): string[] {
...
function kwText(reviewKws: ReviewKeyword[], menuKws: MenuKeyword[]): string {
  return [
...
function blogText(blogReviews: BlogReview[]): string {
...
function inferFoodType(
  category: string, name: string,
  reviewKws: ReviewKeyword[], menuKws: MenuKeyword[],
): string[] {
  const text = `${category} ${name} ${kwText(reviewKws, menuKws)}`;
  return [...new Set(matchKeywords(text, FOOD_TYPE_MAP))];
}

function inferAtmosphere(
  reviewKws: ReviewKeyword[],
...
function inferPriceRange(menuItems: Array<{ name: string; price?: string }>): string[] {
  const prices: number[] = [];
  for (const item of menuItems) {
    if (!item.price) continue;
    const num = parseInt(item.price.replace(/[^0-9]/g, ''), 10);
    if (num > 500 && num < 500_000) prices.push(num);
  }
  if (!prices.length) return [];

  const avg = prices.reduce((a, b) => a + b, 0) / prices.length;
  if (avg < 10_000) return ['1만원미만'];
  if (avg < 20_000) return ['1-2만원'];
  if (avg < 30_000) return ['2-3만원'];
  return ['3만원이상'];
}

function inferMealTime(businessHours: string): string[] {
  ...
  for (const m of ranges) {
    ...
    if (openMin <= 9 * 60)                              tags.push('아침');
    if (openMin <= 12 * 60 && closeMin >= 13 * 60)      tags.push('점심');
    if (closeMin >= 17 * 60)                            tags.push('저녁');
    if (closeMin >= 22 * 60 || closeMin <= 4 * 60)      tags.push('야식');
    if (openMin >= 9 * 60 && openMin <= 11 * 60 && closeMin <= 17 * 60) tags.push('브런치');
    if (openMin <= 7 * 60)                              tags.push('새벽영업');
  }

  if (/24시간|24시|연중무휴/.test(businessHours)) tags.push('24시간');
...
export function autoTagRestaurant(r: RestaurantData): AutoTags {
...
export function flattenAutoTags(tags: AutoTags): string[] {

...
async function debug() {
  ...
  // TYaxT 클래스 요소의 구조 확인
  const tyaxtInfo = await page.$$eval('[class*="TYaxT"]', (els) =>
  ...
  // item 클래스 요소 확인
  const itemInfo = await page.$$eval('[class*="item"]', (els) =>
    els.slice(0, 3).map((el) => ({
      tag: el.tagName,
  ...
  const allHrefs = await page.$$eval('a[href]', (els) =>
    els.map((a) => a.getAttribute('href') ?? '').filter((h) => h.includes('place') || h.includes('restaurant') || h.match(/\/\d{5,}/))
  );
  console.log('\n--- place/restaurant 관련 링크 ---');
  console.log(allHrefs.slice(0, 20));

  // 숫자 ID가 포함된 모든 링크
  const idLinks = await page.$$eval('a[href]', (els) =>
    els.map((a) => a.getAttribute('href') ?? '').filter((h) => /\/\d{7,}/.test(h))
  );
  console.log('\n--- 숫자 ID 포함 링크 ---');
  console.log(idLinks.slice(0, 20));

  // data-cid 또는 data-id 속성 확인
  const dataIds = await page.$$eval('[data-cid], [data-id], [data-sid]', (els) =>
  ...
  const hasApollo = await page.evaluate(() => !!(window as any).__APOLLO_STATE__);
  const hasNext = await page.evaluate(() => !!(window as any).__NEXT_DATA__);
  console.log('\n--- 글로벌 데이터 ---');

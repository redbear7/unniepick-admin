        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={4} className="text-center py-12 text-dim">회원이 없어요</td>
              </tr>
            ) : (
              filtered.map(user => (
                <tr key={user.id} className="border-b border-border-main hover:bg-white/[0.02] transition">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-full bg-fill-medium flex items-center justify-center text-xs font-bold text-tertiary">
                        {user.name.charAt(0)}
                      </div>
                      <p className="font-semibold text-primary">{user.name}</p>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-tertiary">{user.phone ?? '-'}</td>

    ...
    if (waveformStyle === 'bar') {
      ...
      if (freqData && freqData.length > 0 && isPlaying) {
        ...
        for (let i = 0; i < FBARS; i++) {
          ...
          const grad = ctx.createLinearGradient(bx, H / 2 - bh / 2, bx, H / 2 + bh / 2);
          grad.addColorStop(0,   `${accentCss}${aStr}`);
          grad.addColorStop(0.5, `${accentCss}ff`);
          grad.addColorStop(1,   `${accentCss}${lStr}`);
          ctx.fillStyle = grad;
          ctx.fillRect(bx + 0.5, H / 2 - bh / 2, Math.max(fbw - 1.5, 1), bh);
          ctx.fillStyle = `${accentCss}20`;
          ctx.fillRect(bx + 0.5, H / 2 + bh / 2, Math.max(fbw - 1.5, 1), bh * 0.25);
        }
      }
